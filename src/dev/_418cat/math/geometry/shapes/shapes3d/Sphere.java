...
import dev._418cat.math.geometry.lines.Line3D;
...
public class Sphere implements Reflector3D
{
	...
	public Point3D[] intersection(Ray3D ray)
	{
		...
		if (this.intersectionState(ray) == 0)
			return new Point3D[] { this.projection(ray) };

		Point3D projCent = ray.projection(center);

		double distProjCentIntersect = Math
			.sqrt(radius * radius - Math.pow(ray.projection(center).distance(center), 2));
	...
	public Ray3D reflect(Ray3D ray)
	{
		...
		if (closestIntersect == null && this.intersects(ray))
