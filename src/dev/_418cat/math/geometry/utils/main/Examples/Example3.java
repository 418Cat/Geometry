...
public class Example3 implements Example
{
	...
	class Settings
	{
		enum SETTING
		{
			RAY_NB(700), MAX_BOUNCES(20), LIGHT_INTENSITY(200), FOV(360), RAY_ANGLE_OFFSET(1), SHOW_RAYS(0);
	...
	private void resolveEvent(Event<?> event)
	{
		...
		case click:
		{
			scrollSetting = Settings.SETTING.values()[(settings.indexOfSetting(scrollSetting) + 1)
				% Settings.SETTING.values().length];
			break;
		}
		case move:
		{
		...
		case scroll:
		{
			...
			settings.setSettingValue(scrollSetting,
				scrollAmount < 0 ? (int) (settings.getSettingValue(scrollSetting) * 0.9)
					: (int) (settings.getSettingValue(scrollSetting) * 1.1 + 1));
			settings.setSettingValue(scrollSetting,
				settings.getSettingValue(scrollSetting) < 0 ? 0 : settings.getSettingValue(scrollSetting));
	...
	public void render()
	{
		...
		double degreePerRay = (double) settings.getSettingValue(Settings.SETTING.FOV)
			/ (double) settings.getSettingValue(Settings.SETTING.RAY_NB);

		for (int rayNb = 0; rayNb < settings.getSettingValue(Settings.SETTING.RAY_NB); rayNb++)
		{
			float totalDist = 0;

			Ray2D currentRay = new Ray2D(ray.origin,
				ray.vect.turn(-(double) settings.getSettingValue(Settings.SETTING.FOV) / 2 + rayNb * degreePerRay));
			if (settings.getSettingValue(Settings.SETTING.RAY_ANGLE_OFFSET) != 0)
				currentRay.vect = currentRay.vect.turn(Math.random() * degreePerRay);
			...
			for (int bounce = 0; bounce <= settings.getSettingValue(Settings.SETTING.MAX_BOUNCES); bounce++)
			{
				...
				if (closestRef == null)
				{
					if (settings.getSettingValue(Settings.SETTING.SHOW_RAYS) != 0)
						DrawUtils.draw(currentRay, Main.frame);
					break;
				}
				if (totalDist > settings.getSettingValue(Settings.SETTING.LIGHT_INTENSITY))
					break;

				if (settings.getSettingValue(Settings.SETTING.SHOW_RAYS) != 0)
					DrawUtils.draw(new Segment2D(currentRay.origin, closestPoint), Main.frame);

				int intensity = (int) ((settings.getSettingValue(Settings.SETTING.LIGHT_INTENSITY) - totalDist) <= 0.5
					? 0
					: 255 * (settings.getSettingValue(Settings.SETTING.LIGHT_INTENSITY) - totalDist)
						/ settings.getSettingValue(Settings.SETTING.LIGHT_INTENSITY));
				if (intensity > 255)
					System.out.println(intensity);

				DrawUtils.drawPix((int) closestPoint.x, (int) closestPoint.y,
					new Color(intensity, intensity, intensity), Main.frame);
