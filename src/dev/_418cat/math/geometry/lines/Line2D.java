...
public class Line2D implements Reflector2D
{
	...
	public Point2D[] intersection(Line2D line)
	{
		...
		angleIntersectPointProj = angleIntersectPointProj > 90 ? 90 - angleIntersectPointProj % 90
			: angleIntersectPointProj;

		double dist = pointToProj.norm() / Math.cos(angleIntersectPointProj * Math.PI / 180);

		return (angleInf90 ? new Point2D[] { this.vect.unit().multiply(dist).transform(this.point) }
			: new Point2D[] { this.vect.negate().unit().multiply(dist).transform(this.point) });
	}
