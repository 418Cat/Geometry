...
public class Frame {
	...
	public Frame(int[] size, int[] location, eventResolver evResolver)
	{
		...
		{
			throw new IllegalArgumentException("Frame constructor need `int[] size` and `int[] location` parameters to both be of length 2");
		}
	...
	public void start()
	{
		...
		if(CLEAR_EACH_FRAME) g.clearRect(0, 0, size[0], size[1]);
		
		if(showDebug) displayFps();
