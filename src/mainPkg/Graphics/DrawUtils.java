...
public class DrawUtils
{
	...
	{
		if(ref.getClass() == Circle.class) draw((Circle)ref, f);
		if(ref.getClass() == Segment2D.class) draw((Segment2D)ref, f);
		if(ref.getClass() == Line2D.class) draw((Line2D)ref, f);
		if(ref.getClass() == Triangle.class) draw((Triangle)ref, f);
		if(ref.getClass() == Ray2D.class) draw((Ray2D)ref, f);
	}
	...
	public static void draw(Point2D a, String name, Frame f)
	{
		...
		f.g().fillOval((int)((a.x - 5)*f.ZOOM), (int)((a.y - 5)*f.ZOOM), (int)((10*f.ZOOM)), (int)((10*f.ZOOM)));
	...
	public static void draw(Point2D pnt, Vector2D vect, Frame f)
	{
		...
		f.g().drawLine((int)(pnt.x*f.ZOOM), (int)(pnt.y*f.ZOOM), (int)((pnt.x + vect.x)*f.ZOOM), (int)((pnt.y + vect.y)*f.ZOOM));
		f.g().fillOval((int)(((pnt.x + vect.x) - 3)*f.ZOOM), (int)(((pnt.y + vect.y)-3)*f.ZOOM), (int)(float)(6*f.ZOOM), (int)(float)(6*f.ZOOM));
	...
	public static void draw(Line2D line, Frame f)
	{
		...
		f.g().drawLine((int)(line.point.x*f.ZOOM), (int)(line.point.y*f.ZOOM), (int)((line.point.x + line.vect.unit().x*mult)*f.ZOOM), (int)((line.point.y + line.vect.unit().y*mult)*f.ZOOM));
		f.g().drawLine((int)(line.point.x*f.ZOOM), (int)(line.point.y*f.ZOOM), (int)((line.point.x - line.vect.unit().x*mult)*f.ZOOM), (int)((line.point.y - line.vect.unit().y*mult)*f.ZOOM));
	}
	...
	private static void draw(Circle circle, Frame f)
	{
		...
		f.g().drawOval((int)((circle.center.x - circle.radius)*f.ZOOM), (int)((circle.center.y - circle.radius)*f.ZOOM), (int)((circle.radius*2)*f.ZOOM), (int)((circle.radius*2)*f.ZOOM));
	}
	
	public static void draw(Segment2D segA, Segment2D segB, int radius, Frame f)
	{
		f.g().setColor(Color.cyan);
		f.g().drawArc((int)segA.A.x-radius, (int)segA.A.y-radius, radius*2, radius*2, (int)AbsAngle.angle(segA, new Vector2D(1, 0)),(int)AbsAngle.angle(segA, segB));
	}
	
	private static void draw(Segment2D segment, Frame f)
	{
		f.g().setColor(Color.gray);
		f.g().drawLine((int)(segment.A.x*f.ZOOM), (int)(segment.A.y*f.ZOOM), (int)(segment.B.x*f.ZOOM), (int)(segment.B.y*f.ZOOM));
	...
	public static void draw(Ray2D ray, Frame f)
	{
		...
		f.g().drawLine((int)(ray.origin.x*f.ZOOM), (int)(ray.origin.y*f.ZOOM), (int)((ray.origin.x + ray.vect.unit().x*mult)*f.ZOOM), (int)((ray.origin.y + ray.vect.unit().y*mult)*f.ZOOM));
	}
}
